...
| 11 | Encyclopedic — 2000+ leaf elements, 3-level hierarchy, full per-discipline depth | The discipline list is a floor, not a ceiling. Comprehensiveness is the point, not a stretch goal. |
...
| 20 | Certification unit is (element × level); per-element ceilings; L1–L2 at competency-area level, L3+ per element | Not every element supports five levels. Testing foundational knowledge element by element over-decomposes it; the distinctions only start mattering at L3. |
