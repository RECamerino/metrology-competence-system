...
The cost is real and lands in a specific place: **a badly bound archetype tests the archetype rather than the element.** That failure is invisible in the archetype, which may be excellent, and invisible in the binding, which may be well-formed. So binding review is a distinct quality gate with its own reviewers, not a rubber stamp on top of archetype review. **It now has a record — added 2026-09-05.** A binding carries a `review` with the reviewer, their standing, a date and a disposition, and it is REQUIRED before a binding may leave `draft`: a draft is a proposal and needs no signoff, anything beyond it is being served to candidates. The parallel is rule 7, where nothing above L2 rests on a draft element because that is where independent work is entrusted. A `disputed` or `rejected` review is kept with its reason — the next author to reach for that archetype at that level needs to know somebody already tried — and a binding carrying one may not be served. The reviewer may not be an author of the element, for the reason no-self-signoff exists. The `kind` classification is what makes bindings tractable — a `skill` element cannot be bound to a knowledge archetype, and the validator can enforce that mechanically.

**And the other half of that open item: when are two differently parameterized items the same exposure?** Until it was decided, `exposureGroup` was the only lever — an entry carrying one collapsed with every other entry carrying it, and an entry without one counted alone. So a binding collapsed either ALL of its draws into a single exposure or NONE of them, decided by whether an author happened to write a string, and two authors following the same instructions produced banks of different effective sizes. **Two draws are one exposure when they agree on every EXPOSURE-RELEVANT parameter**, which each archetype now declares per parameter and which is deliberately not the same question as `visibility`: a generator parameter carries the structural feature and is almost always exposure-relevant, but a prompt parameter may be either — `measurement_context` varies the surface and nothing else, while `stated_procedure` on the witnessed archetypes changes what the candidate is asked to do and is the difficulty knob between levels. It defaults to TRUE, because over-counting exposures makes the limit bite sooner and under-counting is what lets a candidate meet one shape at every level. `exposureGroup` now namespaces rather than collapses, which is what its own description always said it did.
...
**The same defect sat in `authorization.scope` until 2026-09-05, and it is worth naming as a pattern.** Activities, methods, ranges and locations were four arrays of free prose, so decision 34's scope-matching engine — *does this authorization cover this method, at this range, at this site* — could only be answered by a person reading strings. "Signing certificates", "certificate signing" and "sign accredited certs" are three spellings of one activity that no comparison finds, and a range recorded as `"0.5 mm to 100 mm"` compares against nothing at all. Activities are now a closed enum, methods carry identifiers, ranges carry a quantity, a unit and numeric bounds, and the measurement dimension resolves against the taxonomy using the **same selector definitions a deployment scope uses** — shared in `common.schema.json` rather than copied, because two implementations of "is this element inside this selector set" would eventually disagree.

**Three answers, not two.** `authorizationCovers` returns `covered`, `not-covered` or **`undecidable`**, and the third is what makes the other two safe: collapsing it into `not-covered` wrongly blocks work somebody is authorized to do, and collapsing it into `covered` wrongly permits signing an accredited certificate. Those failures are not symmetrical and neither is acceptable.

**The unit trap is why `undecidable` had to exist.** A grant of 0 to 100 mm and a job at 0.5 m compare as `0.5 < 100` the moment units are ignored — authorizing a certificate five times outside the granted range, with arithmetic that looks perfectly sound. **The project does not ship a unit-conversion table to close that.** A half-right conversion inside a metrology system is worse than none, because it fails silently and in the direction of permitting more. Same quantity, different unit, is a question the engine refuses to answer, and it says which units it was given.
...
- **The pin covers claim state, not only prose — corrected 2026-09-04.** It hashed the section body alone, which pinned what a section SAYS while ignoring what it CLAIMS ABOUT ITSELF. Flip `consensus` from `established` to `contested` and not one byte of prose moves: the hash still matched, so the reviewer's attestation survived a change to how the passage must be read, and a credential resting on it reported no drift while the knowledge behind the claim went from settled to disputed. Those are the opposite of small edits — they are the difference between a reader acting on a passage and a reader knowing they must not. `consensus`, `contestedBasis` and `alternativeViews` are now inside the pin; the last of those matters most, because it carries the substance of the disagreement, is required whenever a section is contested, and can be rewritten in full without touching a word of prose. `covers` and `contestedBasisNote` stay outside as editorial, and `deprecated`/`supersededBy` stay outside as lifecycle — on exactly the argument that keeps an element's `status` out of the definition pin, since superseding a section would otherwise light up every credential ever issued against it. **This changed every section hash**, which cost nothing: no article carried a review and no credential had been issued.
- **Reviewer standing is recorded, and the three bases are told apart — added 2026-09-04.** `reviewer` is a name and an optional DID, so the corpus could establish that X reviewed a section on a date and could not establish that X had any standing to. That is precisely the gap decision 47 closed for credential signers, where an unbacked `heldLevel: 4` stopped counting as evidence of anything, and it had never been applied to the people reviewing the corpus itself. `standing.basis` is `held-credential` (named and pinned, resolvable offline), `founding-cohort` (the reviewer's DID resolves against the published roster) or `stated` (external practice in prose, which a reader WEIGHS rather than resolves). The two resolvable bases are checked and refuse to be claimed without the thing that resolves them. **Nothing can prove a `stated` basis and nothing tries** — what the field buys is that the three stop being indistinguishable. It is also the only basis available while nobody holds a credential and the cohort convenes nobody, so it is the normal case rather than the degraded one, and standing is required only where a claim rests on it: a gold reference may not rest on a name alone.
...
**The suite identifier, though, was invented — and nothing in this repository could ever have caught it.** The schemas fixed `cryptosuite` at `ecdsa-rdfc-2019-p256`. W3C Data Integrity ECDSA Cryptosuites v1.0 defines exactly three identifiers — `ecdsa-rdfc-2019`, `ecdsa-jcs-2019` and `ecdsa-sd-2023` — and none of them carries a curve. The value had been assembled locally out of the right suite family and the right curve, joined by a hyphen, and it read perfectly. A `const` is only ever checked against itself, so CI was never going to notice; it would have been found by the first person to carry a credential to an implementation that was not ours, which is precisely the moment this project cannot afford to fail. A credential the holder owns is worth what an outside verifier can do with it, and nothing else.

**The correction is not a de-hyphenation, because `rdfc` was the wrong half to keep.** `ecdsa-rdfc-2019` transforms by converting the document to JSON-LD expanded form, then to RDF statements, then canonicalizing under RDFC-1.0 — it requires an `@context` and the resolution of one. Nothing this project signs is JSON-LD. There is no `@context` on a credential, an authorization or the trust registry, and adding one would pull a JSON-LD processor and context resolution into a build whose first constraint is that it makes no external runtime calls. `ecdsa-jcs-2019` canonicalizes the JSON directly under RFC 8785: no context, no processor, no network. **The suite is `ecdsa-jcs-2019`**, and it is the one that fits both the documents actually signed and the air gap.

**The curve then needed somewhere real to live.** It is carried by the verification method's Multikey — multicodec `0x1200` for P-256 against `0x1201` for P-384 — and never by the suite name, so the `-p256` suffix was not load-bearing even had the identifier been real. Removing it would have left the FIPS argument above with no expression anywhere in the data. It is now a pattern on `publicKeyMultibase` in the trust registry: a P-256 Multikey is `0x8024` plus a 33-byte compressed point, base58btc-encoded, which is always 49 characters beginning `zDnae`. P-384 encodes to `z82L…` and Ed25519 to `z6Mk…`, and neither is admitted. The bound is derived rather than taken from convention — base58 is order-preserving at fixed length, so the minimum and maximum encodings bracket every valid one between them — and the test recomputes it rather than trusting the note.

**Note what kind of error this was.** Not a metrology judgement and not a design mistake: a fact about the outside world, taken on plausibility, written into a `const` that would have frozen at Phase 3. It is the category the review record already flags as the one that fails — claims about the corpus hold up, claims about the world outside it often do not — and the only defence is to go and read the specification.
...
**Amended 2026-09-20, two days later, by review finding F-04: `account` is required only where the activity does not pin a record instead.** The floor compelled several hundred characters at L5, and the problem was never the length — it was whose information it is. A calibration laboratory's customer, method and results are the CUSTOMER'S commercial information, the holder is not always at liberty to describe it, and the document is built to be handed to a future employer who is often that customer's competitor. A schema that compels the description either extracts a disclosure that was not the holder's to make, or extracts a sentence vague enough to be worthless — which is the outcome the floor existed to prevent.

`evidence[].sufficiency` is not the counter-example it looks like: that is a REVIEWER'S judgement about an artifact and is part of the attestation. This is a description of somebody else's business.

**And nothing can be redacted afterwards.** `ecdsa-jcs-2019` canonicalizes the whole document (ECDSA Cryptosuites §3.3), so removing a field breaks the signature. The suite that permits it is `ecdsa-sd-2023` (§3.6), whose base-proof/derived-proof mechanism works over canonical N-Quads — RDF, an `@context`, and the network resolution rule 5 forbids and decision 17 already rejected `ecdsa-rdfc-2019` for. **Selective disclosure is not merely unchosen in this project; it is unavailable**, so whatever is written travels to everybody, forever.

So an activity owes **an account or a pinned `ref`**, and `demonstrates` either way. That is the arrangement `evidence[]` already uses — the artifact pinned for whoever can reach it, the argument travelling for everybody — and `demonstrates` is the field decision 19 actually exists for: it is written against the element's anchor in the corpus's own vocabulary, so it describes a competence rather than a customer. The signers judged the work with the record in front of them; a later reader gets the hash and the argument. **Decision 19 is intact; what changed is that the holder is no longer compelled to publish somebody else's business to satisfy it.**
