...
| `ARC-0001`–`0006` | `ARC-0004`, `0005` and `0006` span families; `ARC-0001`–`0003` are narrow. **`ARC-0005` is the first witnessed archetype and the first to bind outside `CM-03`** — written for one test, widened to cg-18's family of measurement methods when a second element landed beside it, and the third then bound to it with no schema change and no new test, which is what tells you a family shape from a convenient one. **`ARC-0006` is the second witnessed archetype, for a different equipment class**, and it generalizes along a different axis — read the two together before writing a third |
...
**But a witnessed archetype spans an EQUIPMENT CLASS, not the axis.** The coverage report said so before anybody worked it out: `EC-01-030` had been authored since August and had no items, and after `ARC-0005` existed it still had none — because that archetype is a weighing-instrument shape and the procedures it spans are cg-18's. So the `EC` axis needs roughly **one witnessed archetype per equipment-class family**: far fewer than 2732, considerably more than one. That is a Phase 7 estimate nobody had made, and it is the first number the item bank has produced about its eventual size rather than its current coverage.

**`ARC-0006` was written to test that, and it refines it in a way worth knowing before a third is attempted.** The two witnessed archetypes are recognizably related and generalize along **different axes**. `ARC-0005` spans several TESTS on one instrument, because cg-18 defines three distinct methods for a balance. `ARC-0006` spans one test across several FUNCTIONS, because verifying a multimeter is the same procedure applied to DC voltage, then current, then resistance. **The family is whatever an equipment class's procedures actually share, and it is found per class rather than derived from a template** — so the count generalizes and the shape does not. Anybody writing the third should expect to discover its axis rather than assume one.
