...
| Checks | 0 errors · 304/304 tests · typecheck clean |
...
| `common` | Shared `$defs` — every ID pattern lives here, plus `organizationRef`, because deciding whether two organizations differ is a rule and not a display concern, `person`, because articles and elements both have to name the humans behind them, and `reviewerStanding`, because a reviewer's standing is the same question wherever it is asked |
